                       ...
                       bool throwError, bool *rval)
{
    ...
    if (!shape) {
        ...
        if (!JSObject::isExtensible(cx, obj, &extensible))
            return false;
        if (!extensible)
            return Reject(cx, (JSObject*) obj, JSMSG_OBJECT_NOT_EXTENSIBLE, throwError, rval);
                      ...
                      bool throwError, bool *rval)
{
    ...
    if (js_IdIsIndex(id, &index)) {
        ...
        if (index >= oldLen && !arr->lengthIsWritable())
            return Reject(cx, (JSObject*) arr, JSMSG_CANT_APPEND_TO_ARRAY, throwError, rval);
...
js::DefineProperties(JSContext *cx, HandleObject obj, HandleObject props)
{
    ...
    if (obj->getOps()->lookupGeneric) {
        ...
        bool dummy;
        return Reject(cx, (JSObject*) obj, JSMSG_OBJECT_NOT_EXTENSIBLE, true, &dummy);
    }
