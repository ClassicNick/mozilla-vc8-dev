...
js_FindClassObject(JSContext *cx, JSProtoKey protoKey, MutableHandleValue vp, Class *clasp)
{
    ...
    RootedObject pobj(cx);
    RootedShape shape(cx);
    if (!LookupPropertyWithFlagsId(cx, cx->global(), id, 0, &pobj, &shape))
        return false;
    RootedValue v(cx, UndefinedValue());
                   ...
                   MutableHandle<Value> vp)
{
    ...
    if (shape->hasSlot() && pobj->nativeContainsShape(cx, shape))
             ...
             HandleShape shape, bool added, bool strict, Value *vp)
{
    ...
    if (shape->hasSlot() &&
        (JS_LIKELY(cx->runtime->propertyRemovals == sample) ||
         obj->nativeContainsShape(cx, shape))) {
        AddTypePropertyId(cx, obj, shape->propid(), *vp);
                            ...
                            MutableHandleValue vp)
{
    RootedShape prop(cx);
    RootedObject obj2(cx);
    if (!LookupPropertyWithFlagsId(cx, obj, id, 0, &obj2, &prop))
        return false;
...
JSBool
js::GetMethodId(JSContext *cx, HandleObject obj, HandleId id, unsigned getHow, MutableHandleValue vp)
{
    JSAutoResolveFlags rf(cx, 0);
...
JSObject::callMethod(JSContext *cx, HandleId id, unsigned argc, Value *argv, MutableHandleValue vp)
{
    ...
    return GetMethodId(cx, obj, id, 0, &fval) &&
