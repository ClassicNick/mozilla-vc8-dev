        ...
        if self.descriptor.interface.hasInterfacePrototypeObject():
            return indent(
                ...
                fill(
                    ...
                    MOZ_STATIC_ASSERT((IsBaseOf<nsISupports, ${nativeType}>::value),
            ...
            if m.isAttr() and not m.isStatic() and m.type.isSerializable():
                ret += fill(
                    ...
                    { // scope for "temp"
                      ...
                      if (!JS_DefineProperty(cx, result, "${name}", (JS::HandleValue) temp, JSPROP_ENUMERATE)) {
                        return false;
                      }
                    }
                    """,
...
class CGSpecializedReplaceableSetter(CGSpecializedSetter):
    ...
    def definition_body(self):
        ...
        assert all(ord(c) < 128 for c in attrName)
        return indent('return JS_DefineProperty(cx, obj, "%s", (JS::HandleValue) args[0], JSPROP_ENUMERATE);\n' %
                      attrName)
