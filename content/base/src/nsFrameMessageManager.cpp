                       ...
                       ClonedMessageData& aClonedData)
{
  ...
  if (!blobs.IsEmpty()) {
    ...
    for (uint32_t i = 0; i < length; ++i) {
      ...
      blobList.AppendElement((ProtocolType*) protocolActor);
    }
  }
  return true;
}
...
UnpackClonedMessageData(const ClonedMessageData& aData)
{
  ...
  if (!blobs.IsEmpty()) {
    ...
    for (uint32_t i = 0; i < length; ++i) {
      typename BlobTraits<Flavor>::BlobType* blob =
...
nsFrameMessageManager::GetDelayedFrameScripts(JSContext* aCx, JS::MutableHandle<JS::Value> aList)
{
  ...
  for (uint32_t i = 0; i < mPendingScripts.Length(); ++i) {
	...
	NS_ENSURE_TRUE(JS_SetElement(aCx, array, i, (JS::HandleObject) pair),
  ...
  for (uint32_t i = 0; i < len; ++i) {
    ...
    if (!JS_ParseJSON(aCx, static_cast<const jschar*>(retval[i].get()),
      ...
      return NS_ERROR_UNEXPECTED;
					  ...
					  NS_ENSURE_TRUE(JS_SetElement(aCx, dataArray, i, (JS::HandleValue) ret),
                   NS_ERROR_OUT_OF_MEMORY);
  }
