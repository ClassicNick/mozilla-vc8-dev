...
public:
                      ...
                      TrackTicks aMaxPos)
  {
    ...
    uint32_t numFrames =
      NS_MIN<TrackTicks>(WEBAUDIO_BLOCK_SIZE - *aOffsetWithinBlock,
                      ...
                      uint32_t aBufferMax)
  {
    ...
    uint32_t numFrames =
      NS_MIN<TrackTicks>(NS_MIN(WEBAUDIO_BLOCK_SIZE - *aOffsetWithinBlock,
...
AudioBufferSourceNode::SendOffsetAndDurationParametersToStream(AudioNodeStream* aStream)
{
  ...
  int32_t offsetSamples = NS_MAX(0, NS_lround(mOffset * rate));
