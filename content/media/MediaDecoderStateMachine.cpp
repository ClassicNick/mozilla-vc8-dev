...
#define DECODER_LOG(type, msg, a) \
  PR_LOG(gMediaDecoderLog, type, ("Decoder=%p " msg, mDecoder.get(), ##a))
#define VERBOSE_LOG(msg, a)                          \
    PR_BEGIN_MACRO                                     \
      if (!PR_GetEnv("MOZ_QUIET")) {                   \
        DECODER_LOG(PR_LOG_DEBUG, msg, ##a); \
      }                                                \
    PR_END_MACRO
#else
#define DECODER_LOG(type, msg, a)
...
void MediaDecoderStateMachine::Seek(const SeekTarget& aTarget)
{
  ...
  seekTime = NS_MIN(seekTime, mEndTime);
  seekTime = NS_MAX(mStartTime, seekTime);
  ...
  switch (mState) {
    ...
    case DECODER_STATE_COMPLETED: {
      ...
      if (mDecoder->GetState() == MediaDecoder::PLAY_STATE_PLAYING &&
        ...
        int64_t clockTime = NS_MAX(mEndTime, std::max(videoTime, GetAudioClock()));
