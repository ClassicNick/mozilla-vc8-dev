  ...
  for (nsBlockFrame* curFrame = this; curFrame;
         ...
         line != line_end; ++line)
    {
      ...
      if (line->IsBlock()) {
        ...
        *aX = NS_MIN(*aX, childX);
        *aXMost = NS_MAX(*aXMost, childXMost);
      } else {
        if (!curFrame->GetPrevContinuation() &&
            line == curFrame->begin_lines()) {
          // Only add text-indent if it has no percentages; using a
        ...
        for (int32_t i = 0, i_end = line->GetChildCount(); i != i_end;
          ...
          rv = kid->GetPrefWidthTightBounds(aRenderingContext, &childX,
                                            &childXMost);
          NS_ENSURE_SUCCESS(rv, rv);
          *aX = NS_MIN(*aX, data.currentLine + childX);
          *aXMost = NS_MAX(*aXMost, data.currentLine + childXMost);
          kid->AddInlinePrefWidth(aRenderingContext, &data);
        }
      }
    }
...
nsBlockFrame::ReflowDirtyLines(nsBlockReflowState& aState)
{
  ...
  if (HasOutsideBullet() && mLines.empty()) {
    ...
    if (!BulletIsEmpty()) {
      ...
      aState.mY += NS_MAX(minAscent, metrics.TopAscent()) +
                   NS_MAX(minDescent, metrics.Height() - metrics.TopAscent());
                        ...
                        bool*             aKeepReflowGoing)
{
  ...
  // checking.
  aAvailableSpaceHeight = NS_MAX(aAvailableSpaceHeight, aLine->BSize());
