...
nsColumnSetFrame::GetAvailableContentHeight(const nsHTMLReflowState& aReflowState)
{
  ...
  return NS_MAX(0, aReflowState.AvailableHeight() - bp.TopBottom());
}
...

{
  ...
  if (aReflowState.ComputedHeight() != NS_INTRINSICSIZE) {
    ...
    colHeight = NS_MIN(colHeight, aReflowState.ComputedMaxHeight());
  }
  ...
  if (colStyle->mColumnWidth.GetUnit() == eStyleUnit_Coord) {
    ...
    if (availContentWidth != NS_INTRINSICSIZE && colGap + colWidth > 0
      ...
      // want
      int32_t maxColumns =
        NS_MIN(nscoord(nsStyleColumn::kMaxColumnCount),
  ...
  if (colWidth != NS_INTRINSICSIZE && availContentWidth != NS_INTRINSICSIZE) {
    ...
    if (numColumns <= 0) {
      ...
      if (colGap + colWidth > 0) {
        ...
        numColumns = NS_MIN(nscoord(nsStyleColumn::kMaxColumnCount),
                              numColumns);
      }
      if (numColumns <= 0) {
        numColumns = 1;
  ...
  if (isBalancing) {
    ...
    colHeight = NS_MAX(colHeight, nsPresContext::CSSPixelsToAppUnits(1));
  }
                                 ...
                                 ColumnBalanceData&       aColData)
{
  ...
  if (aConfig.mComputedHeight != NS_INTRINSICSIZE) {
    if (aReflowState.AvailableHeight() != NS_INTRINSICSIZE) {
      contentSize.height = NS_MIN(contentSize.height,
                                        ...
                                        nsReflowStatus& aStatus)
{
  ...
  while (!aPresContext->HasPendingInterrupt()) {
    ...
    if (feasible) {
      ...
      aConfig.mKnownFeasibleHeight = NS_MIN(aConfig.mKnownFeasibleHeight,
                                              aColData.mMaxHeight);
      aConfig.mKnownFeasibleHeight = NS_MIN(aConfig.mKnownFeasibleHeight,
                                              mLastBalanceHeight);
      ...
      if (mFrames.GetLength() == aConfig.mBalanceColCount) {
        aConfig.mKnownInfeasibleHeight = NS_MAX(aConfig.mKnownInfeasibleHeight,
