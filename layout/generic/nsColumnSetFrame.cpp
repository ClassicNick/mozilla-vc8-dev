  ...
  if (config.mIsBalancing && !aPresContext->HasPendingInterrupt()) {
    ...
    while (!aPresContext->HasPendingInterrupt()) {
      ...
      if (feasible) {
        ...
        config.mKnownFeasibleHeight = NS_MIN(config.mKnownFeasibleHeight,
                                               colData.mMaxHeight);
        config.mKnownFeasibleHeight = NS_MIN(config.mKnownFeasibleHeight,
                                               mLastBalanceHeight);
        ...
        if (mFrames.GetLength() == config.mBalanceColCount) {
          config.mKnownInfeasibleHeight =
            NS_MAX(config.mKnownInfeasibleHeight, colData.mLastHeight - 1);
        }
      } else {
        config.mKnownInfeasibleHeight =
          NS_MAX(config.mKnownInfeasibleHeight, mLastBalanceHeight);
        ...
        config.mKnownInfeasibleHeight =
          NS_MAX(config.mKnownInfeasibleHeight,
