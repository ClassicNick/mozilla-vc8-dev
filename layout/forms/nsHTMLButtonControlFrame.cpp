  ...
  const nsMargin& aFocusPadding)
{
  ...
  adjustedWidth = NS_MAX(0, adjustedWidth);
  ...
  if (adjustedHeight != NS_INTRINSICSIZE) {
    ...
    adjustedHeight = NS_MAX(0, adjustedHeight);
  }
                                               ...
                                               nsIFrame* aFirstKid)
{
  ...
  if (extrawidth > 0) {
    ...
    extraleft = NS_MIN(extraleft, aButtonReflowState.ComputedPhysicalPadding().left);
    extraright = NS_MIN(extraright, aButtonReflowState.ComputedPhysicalPadding().right);
    xoffset -= extraleft;
  ...
  nscoord yoffset = NS_MAX(0, extraSpace / 2);
