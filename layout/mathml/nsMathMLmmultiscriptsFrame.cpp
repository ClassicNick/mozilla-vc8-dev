                                        ...
                                        nscoord              aScriptSpace)
{
  ...
  if (tag == nsGkAtoms::msub_) {
    ...
    subScriptShift = NS_MAX(subScriptShift1, subScriptShift2);
  }
  if (0 < aUserSubScriptShift) {
    // the user has set the subscriptshift attribute
    subScriptShift = NS_MAX(subScriptShift, aUserSubScriptShift);
  }
  ...
  if (0 < aUserSupScriptShift) {
    ...
    supScriptShift = NS_MAX(supScriptShift, aUserSupScriptShift);
  }
  ...
  while (childFrame) {
    ...
    if (childTag == nsGkAtoms::mprescripts_) {
      ...
      if (isSubScript) {
        ...
        multiSubSize.SetTopAscent(NS_MAX(multiSubSize.TopAscent(),
                                       subScriptSize.TopAscent()));
        bmMultiSub.ascent = NS_MAX(bmMultiSub.ascent, bmSubScript.ascent);
        bmMultiSub.descent = NS_MAX(bmMultiSub.descent, bmSubScript.descent);
        multiSubSize.Height() = 
          NS_MAX(multiSubSize.Height(),
        ...
        if (tag == nsGkAtoms::msub_) {
          ...
          maxSubScriptShift = NS_MAX(trySubScriptShift,minShiftFromXHeight);
        ...
        trySupScriptShift = NS_MAX(minSupScriptShift,
                                     NS_MAX(minShiftFromXHeight,
                                              supScriptShift));
        multiSupSize.SetTopAscent(NS_MAX(multiSupSize.TopAscent(),
                                       supScriptSize.TopAscent()));
        bmMultiSup.ascent = NS_MAX(bmMultiSup.ascent, bmSupScript.ascent);
        bmMultiSup.descent = NS_MAX(bmMultiSup.descent, bmSupScript.descent);
        multiSupSize.Height() = 
          NS_MAX(multiSupSize.Height(),
                   supScriptSize.Height() - supScriptSize.TopAscent());

        if (bmSupScript.width)
          width = NS_MAX(width, bmSupScript.width + aScriptSpace);

        if (!prescriptsFrame) { // we are still looping over base & postscripts
          rightBearing = NS_MAX(rightBearing,
  ...
  if (tag == nsGkAtoms::msub_) {
    boundingMetrics.ascent = NS_MAX(bmBase.ascent,
                                      bmMultiSub.ascent - maxSubScriptShift);
  } else {
    boundingMetrics.ascent =
      NS_MAX(bmBase.ascent, (bmMultiSup.ascent + maxSupScriptShift));
  }
  if (tag == nsGkAtoms::msup_) {
    boundingMetrics.descent = NS_MAX(bmBase.descent,
                                       bmMultiSup.descent - maxSupScriptShift);
  } else {
    boundingMetrics.descent =
      NS_MAX(bmBase.descent, (bmMultiSub.descent + maxSubScriptShift));
  }
  aFrame->SetBoundingMetrics(boundingMetrics);

  // get the reflow metrics ...
  aDesiredSize.SetTopAscent( 
    NS_MAX(baseSize.TopAscent(), 
             NS_MAX(multiSubSize.TopAscent() - maxSubScriptShift,
                      multiSupSize.TopAscent() + maxSupScriptShift)));
  aDesiredSize.Height() = aDesiredSize.TopAscent() +
    NS_MAX(baseSize.Height() - baseSize.TopAscent(),
             NS_MAX(multiSubSize.Height() + maxSubScriptShift,
