                                    ...
                                    float aOpacity)
{
  ...
  if (nativeTheme) {
    ...
    skipGfxRect.Deflate(gfxMargin(
        NS_MAX(borderRadii[C_TL].height, borderRadii[C_TR].height), 0,
        NS_MAX(borderRadii[C_BL].height, borderRadii[C_BR].height), 0));
  }
                                    ...
                                    const nsRect& aDirtyRect)
{
  ...
  for (uint32_t i = shadows->Length(); i > 0; --i) {
    ...
    if (hasBorderRadius) {
      skipGfxRect.Deflate(gfxMargin(
          NS_MAX(clipRectRadii[C_TL].height, clipRectRadii[C_TR].height), 0,
          NS_MAX(clipRectRadii[C_BL].height, clipRectRadii[C_BR].height), 0));
    }
                                         ...
                                         gfxFloat aScaleY)
{
  ...
  return gfxPoint(NS_MIN((blurStdDev * aScaleX),
