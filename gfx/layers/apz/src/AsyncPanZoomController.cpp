...
#define APZC_LOG_FM(fm, prefix, a) \
  APZC_LOG(prefix ":" \
...
bool FlingAnimation::Sample(FrameMetrics& aFrameMetrics,
  ...
  if (!IsZero(overscroll)) {
    ...
    if (treeManagerLocal) {
      ...
      // released.
      mDeferredTasks.append((Task*) NewRunnableMethod(treeManagerLocal,
...
void AsyncPanZoomController::ZoomToRect(CSSRect aRect) {
...

  {
    ...
    CSSToParentLayerScale localMinZoom(NS_MAX((mZoomConstraints.mMinZoom * mFrameMetrics.mTransformScale).scale,
                                       NS_MAX(compositionBounds.width / cssPageRect.width,
    ...
    if (!aRect.IsEmpty()) {
      ...
      targetZoom = CSSToParentLayerScale(NS_MIN(compositionBounds.width / aRect.width,
...
bool AsyncPanZoomController::TouchActionAllowPinchZoom() {
  ...
  for (size_t i = 0; i < mTouchBlockState.mAllowedTouchBehaviors.Length(); i++) {
    if (!(mTouchBlockState.mAllowedTouchBehaviors[i] & PINCH_ZOOM)) {
      return false;
    }
  }
  return true;
...
bool AsyncPanZoomController::TouchActionAllowDoubleTapZoom() {
  ...
  for (size_t i = 0; i < mTouchBlockState.mAllowedTouchBehaviors.Length(); i++) {
    if (!(mTouchBlockState.mAllowedTouchBehaviors[i] & DOUBLE_TAP_ZOOM)) {
      return false;
    }
  }
  return true;
