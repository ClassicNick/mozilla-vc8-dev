...
GfxInfo::GetCleartypeParameters(nsAString & aCleartypeParams)
{
  ...
  for (d = 0; d < numDisplays; d++) {
    ...
    if (displayNames) {
      swprintf(valStr, ArrayLength(valStr),
...
GfxInfo::Init()
{
  ...
  if (setupapi) {
    ...
    if (setupGetClassDevs &&
      ...
      HDEVINFO devinfo = setupGetClassDevs(nullptr, mDeviceID.get(), nullptr,
        ...
        while (setupEnumDeviceInfo(devinfo, memberIndex++, &devinfoData)) {
          ...
          if (setupGetDeviceRegistryProperty(devinfo,
            ...
            if (result == ERROR_SUCCESS) {
              ...
              result = RegQueryValueExW(key, L"DriverVersion", nullptr, nullptr, (LPBYTE)value, &dwcbData);
              if (result == ERROR_SUCCESS) {
            ...
            mDriverVersion.AssignLiteral("0.0.0.0");
              ...
              dwcbData = sizeof(value);
              result = RegQueryValueExW(key, L"DriverDate", nullptr, nullptr, (LPBYTE)value, &dwcbData);
              if (result == ERROR_SUCCESS) {
            mDriverDate = value;
          } else {
      ...
      if (hresult == NOERROR) {
        devinfo = setupGetClassDevs(&GUID_DISPLAY_DEVICE_ARRIVAL, nullptr, nullptr,
        ...
        if (devinfo != INVALID_HANDLE_VALUE) {
          ...
          while (setupEnumDeviceInfo(devinfo, memberIndex++, &devinfoData)) {
            ...
            if (setupGetDeviceRegistryProperty(devinfo,
              ...
              if (result == ERROR_SUCCESS) {
                dwcbData = sizeof(value);
                result = RegQueryValueExW(key, L"MatchingDeviceId", nullptr, nullptr, (LPBYTE)value, &dwcbData);
                if (result != ERROR_SUCCESS) {
                  continue;
                }
                deviceID2 = value;
                ...
                driverDate2 = value;
                dwcbData = sizeof(value);
                result = RegQueryValueExW(key, L"Device Description", nullptr, nullptr, (LPBYTE)value, &dwcbData);
                if (result != ERROR_SUCCESS) {
                  dwcbData = sizeof(value);
                  result = RegQueryValueExW(key, L"DriverDesc", nullptr, nullptr, (LPBYTE)value, &dwcbData);
                }
                RegCloseKey(key);
                if (result == ERROR_SUCCESS) {
                  mHasDualGPU = true;
