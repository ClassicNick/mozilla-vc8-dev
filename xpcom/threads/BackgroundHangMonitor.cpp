...
BackgroundHangManager::RunMonitorThread()
{
  ...
  while (!mShutdown) {
    ...
    for (BackgroundHangThread* currentThread = mHangThreads.getFirst();
      ...
      recheckTimeout = NS_MIN(recheckTimeout, nextRecheck - hangTime);

      /* We wait for a quarter of the shortest timeout
         value to give mIntervalNow enough granularity. */
      waitTime = NS_MIN(waitTime, currentThread->mTimeout / 4);
    }
  }
...
BackgroundHangThread::ReportHang(PRIntervalTime aHangTime)
{
  ...
  Telemetry::HangHistogram newHistogram(OldMove(mHangStack));
  ...
  mStats.mHangs.append(OldMove(newHistogram));
}
